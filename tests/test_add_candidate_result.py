...
def test_add_candidate_result(engine: Engine) -> None:
    add_candidate_result(engine, "John Smith", "Conservative", 100, 300, 20, "Wetherby", 2024)

    (candidate_result_row1,) = select_results(engine=engine, election=2024)

    assert (
        candidate_result_row1.candidate_name == "John Smith"
        and candidate_result_row1.party_name == "Conservative"
